...
class MediaManager extends Extension
{
    ...
    public function urls()
    {
        return [
            'path'       => $this->path,
            'index'      => route('resources-index'),
            'move'       => route('resources-move'),
            'delete'     => route('resources-delete'),
            'upload'     => route('resources-upload'),
            'new-folder' => route('resources-new-folder'),
        ];
    }
    ...
    public function formatFiles($files = [])
    {
        $files = array_map(function ($file) {
            return [
                'download' => route('resources-download', compact('file')),
                'icon'     => '',
                'name'     => $file,
                'preview'  => $this->getFilePreview($file),
                'isDir'    => false,
                'size'     => $this->getFilesize($file),
                'link'     => route('resources-download', compact('file')),
    ...
    {
        $url = route('resources-index', ['path' => '__path__', 'view' => request('view')]);
        ...
        $dirs = array_map(function ($dir) use ($preview) {
            return [
                ...
                'isDir'    => true,
                'size'     => '',
                'link'     => route('resources-index', ['path' => '/' . trim($dir, '/'), 'view' => request('view')]),
                'url'      => $this->isLocal ? $this->storage->url($dir) : '',
                'time'     => $this->isLocal ? $this->getFileChangeTime($dir) : '',
            ];
        }, $dirs);
    ...
    public function navigation()
    {
        ...
        foreach ($folders as $folder) {
            ...
            $navigation[] = [
                'name' => $folder,
                'url'  => route('resources-index', ['path' => $path]),
            ];
        }
    ...
    public function getFileChangeTime($file)
    {
        if ($this->isLocal) {
            ...
            $time = $this->storage->getTimestamp($file);
        }
